...
uses
  ...
  Vcl.Imaging.pngimage, uResourceString;
...
begin
  ...
  begin
    if txtID.Text <> '' then
    try
      InjectTelegramBot1.SendDice(MyChatId, TtdEmojiType.etFootball ,False, 0, False, LMarkup);
      except on e:exception do
      begin
        memConsole.Lines.Add(VRes_Falha+VRes_Basquete);
      end;
...
begin
  ...
  if AMessage.ReplyToMessage <> NIl then
    memConsole.Lines.Add(
      ...
      ' ID MSG Respondida : '+AMessage.ReplyToMessage.From.ID.ToString+sLineBreak+
      ' Resposta : ' + AMessage.Text+sLineBreak+
      ' Menssagem Respondida : ' + AMessage.ReplyToMessage.Text);
