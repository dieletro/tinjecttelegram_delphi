...
type
  ...
  TInjectTelegramBotChat = class;
  TNotifyTelegramBotConversa = procedure(Conversa: TInjectTelegramBotChat; AMessage: ItdMessage) of object;
  ...
  TInjectTelegramBotChat = class(TComponent)
  private
    //Enumerado
...
{ TInjectTelegramBotChat }

procedure TInjectTelegramBotChat.Clear;
begin
...
function TInjectTelegramBotChat.CarregarBTStr(AStrArrayBtName: TArray<TArray<String>>;AInlineMode: Boolean = false) : IReplyMarkup;
var
  {$REGION 'VARIAVEIS'}
  I, O: Integer;
  str: String;
...
function TInjectTelegramBotChat.CarregarBotoes(AQuery: TFDQuery; AFieldName: String; AInitZero: Boolean = True) : IReplyMarkup;
var
  {$REGION 'VARIAVEIS'}
  I: Integer;
  strTexto : String;
...
constructor TInjectTelegramBotChat.Create(AOwner: TComponent);
begin
...
destructor TInjectTelegramBotChat.Destroy;
begin
  FTimerSleep.Free;
  inherited Destroy;
end;

procedure TInjectTelegramBotChat.ReiniciarTimer;
begin
...
//    FTimerSleep.OnTimer(Self);
  end;
end;

procedure TInjectTelegramBotChat.SetSituacao(const Value: TtdSituacaoAtendimento);
begin
  //DoChange
  if FSituacao <> Value then
  begin
    FSituacao := Value;
    FTimerSleep.Enabled := ((FSituacao = saNova) or (FSituacao = saEmAtendimento));  //Habilita Time se situacao ativa.
...
procedure TInjectTelegramBotChat.SetTempoInatividade(const Value: Integer);
begin
  FTempoInatividade := Value;
  FTimerSleep.Interval := FTempoInatividade;
end;

procedure TInjectTelegramBotChat.TimerSleepExecute(Sender: TObject);
begin
