...
type
  TForm1 = class(TForm)
    ...
    procedure InjectTelegramReceiverService1InlineQuery(ASender: TObject;
      AInlineQuery: ItgInlineQuery);
...
//  MyPrices[0] :=  TtgLabeledPrice.Create('Açucar Mascavo', 100);
  ...
  begin
    ...
    try
      ...
      InjectTelegram1.SendInvoice(
        MyChatId.ID,
        'Titulo',
        'Teste de Descrição',
        'LMCODE-Delivery', //'Playload', //Nome que aparece na Fatura do Cartao
...
begin
  AbrirArquivo.InitialDir := ExtractFilePath(Application.ExeName);
  AbrirArquivo.Filter := 'Stiker|*.jpeg;*.jpg;*.gif;*.png;*.bmp';
  if AbrirArquivo.Execute then
    MyFile := TtgFileToSend.Create(TtgFileToSendTag.FromFile,AbrirArquivo.FileName);
...
begin
  ...
  Begin
    if txtID.Text <> '' then
    try
      InjectTelegram1.SendMessage(StrToint(txtID.Text),'Esta é uma mensagem de exemplo', LParseMode, False, False, 0, LMarkup)
    except on E: Exception do
      memConsole.Lines.Add('Falha no Envio para este destinatáio  - ' + E.Message);
    end;
  End
...
LMarkup := TtgReplyKeyboardMarkup.Create([
  ...
  { Segunda Linha }
  [TtgKeyboardButton.Create('texto3', FALSE, FALSE)]], TRUE);
...
begin
  ...
  begin
    InjectTelegram1.AnswerCallbackQuery(ACallbackQuery.Id, 'texto1RetornoEmbutido!');
  end
  else if ACallbackQuery.data.ToLower.Equals('texto2retornoembutido') then
  begin
    InjectTelegram1.AnswerCallbackQuery(ACallbackQuery.Id, 'texto2RetornoEmbutido!');
  end
  else if ACallbackQuery.data.ToLower.Equals('texto3retornoembutido') then
  begin
    InjectTelegram1.AnswerCallbackQuery(ACallbackQuery.Id, 'texto3RetornoEmbutido!');
  end;
...
procedure TForm1.InjectTelegramReceiverService1InlineQuery(ASender: TObject;
  AInlineQuery: ItgInlineQuery);
begin
  if AInlineQuery.Query.ToLower.Equals('texto2retornoembutido') then
    memConsole.Lines.Add('O Usuario : '+
      AInlineQuery.From.FirstName+' Solicitou '+
      AInlineQuery.Query);
end;
...
procedure TForm1.InjectTelegramReceiverService1Message(ASender: TObject;
  AMessage: ITgMessage);
begin
  if AMessage.Text.ToLower.Equals('texto1') then
    memConsole.Lines.Add('O Usuario : '+
  ...
  if AMessage.Text.ToLower.Equals('texto3') then
    memConsole.Lines.Add('O Usuario : '+
      AMessage.From.FirstName+' Solicitou '+
      AMessage.Text);
end;
