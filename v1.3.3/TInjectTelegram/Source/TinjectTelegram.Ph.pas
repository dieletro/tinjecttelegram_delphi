unit TinjectTelegram.Ph;

interface

uses
  TinjectTelegram.Ph.Types,
  TinjectTelegram.Bot.Impl;

type
  TInjectTelegramPh = class(TInjectTelegramBotBase)
  protected
...
function TInjectTelegramPh.createAccount(const ShortName, AuthorName, AuthorUrl: string):
  IphAccount;
begin
...
function TInjectTelegramPh.createPage(const AccessToken, title, author_name, author_url:
...
procedure TInjectTelegramPh.DoInitApiCore;
begin
  inherited;
...
function TInjectTelegramPh.editAccountInfo(const AccessToken, ShortName, AuthorName,
...
function TInjectTelegramPh.editPage(const AccessToken, Path, title: string; const
...
function TInjectTelegramPh.getAccountInfo(const AccessToken: string): IphAccount;
begin
  Result := TphAccount.Create(//
...
function TInjectTelegramPh.getPage(const path: string; const return_content: Boolean): IphPage;
begin
  Result := TphPage.Create(//
...
function TInjectTelegramPh.getPageList(const access_token: string; const offset, limit:
  Integer): IphPageList;
begin
...
function TInjectTelegramPh.getViews(const path: string; const year, month, day, hour:
...
function TInjectTelegramPh.revokeAccessToken(const AccessToken: string): IphAccount;
begin
  Result := TphAccount.Create(//
