...
{$REGION 'Getting updates'}
    ...
    function SetWebhook( //
      ...
      const DropPendingUpdates:	Boolean = False;
      const SecretToken: string = ''): Boolean;
    /// <summary>
...
{$REGION 'Basic methods'}
    ...
    function SendMessage( //
      ...
      const LinkPreviewOptions : ItdLinkPreviewOptions = nil; //
      const DisableNotification: Boolean = False; //
      const ReplyParameters: ItdReplyParameters = nil; //
      const ReplyMarkup: IReplyMarkup = nil;
      const ProtectContent: Boolean = False;
      const MessageThreadId: int64 = 0;
    ...
    /// Unique identifier for the target message thread (topic) <br/>
    ///  of the forum; for forum supergroups only. <br/>
    /// </param>
    /// <param name="DisableNotification">
    ...
    function ForwardMessage( //
      ...
      const MessageThreadId: Integer;
      const DisableNotification: Boolean = False;
      const ProtectContent: Boolean = False): ItdMessage;
    ///<summary >
    /// Use this method to forward multiple messages of any kind.<br/>
    /// </summary>
    /// <param name="ChatId">
    /// Unique identifier for the target chat or username of the target
    /// channel (in the format @channelusername) <br/>
    ...
    /// Identifiers of 1-100 messages in the chat from_chat_id to forward. <br/>
    ///  The identifiers must be specified in a strictly increasing order. <br/>
    /// </param>
    /// <param name="MessageThreadId">
    /// Unique identifier for the target message thread (topic) <br/>
    ///  of the forum; for forum supergroups only. <br/>
    ...
    /// </param>
    /// <remarks>
    ///  If some of the specified messages can't be found or forwarded, <br/>
    ///  they are skipped. Service messages and messages with protected <br/>
    ///  content can't be forwarded. Album grouping is kept for forwarded <br/>
    ///  messages <br/>
    /// </remarks>
    /// <returns>
    ...
    function SendPhoto( //
      ...
      const HasSpoiler: boolean = False;
      const BusinessConnectionId: string = ''): ItdMessage;
    /// <summary>
    ...
    function SendAnimation( //
      ...
      const Width: Int64 = 0; //
      const Height: Int64 = 0; //
      const Thumbnail: TtdFileToSend = nil; //
      const Caption: string = ''; //
      const ParseMode: TtdParseMode = TtdParseMode.Default; //
      const DisableNotification: Boolean = False; //
      const ReplyParameters: ItdReplyParameters = nil;
    ...
    function SendPoll(
      ...
      const Options: TArray<TtdInputPollOption>; //
      ...
      const ExplanationEntities: TArray<TtdMessageEntity> = []; //
      ...
      const QuestionParseMode: string = '';
      const QuestionEntities: TArray<TtdMessageEntity> = []): ItdMessage;
    /// <summary>
    /// sendDice
    ...
    function SendChatAction( //
      const ChatId: TtdUserLink; //
      const Action: TtdSendChatAction;
      const MessageThreadId: int64 = 0;
    ...
    function setMessageReaction( //
      const ChatId: TtdUserLink; //
      const MessageId: int64; //
      const Reaction: TArray<TtdReactionType>;
      const IsBig: boolean = false): Boolean;
    /// <summary>
    ...
    function sendMediaGroup( //
      ...
      const ProtectContent: Boolean = False;
      const MessageThreadId: int64 = 0;
      const BusinessConnectionId: string = ''): TArray<ItdMessage>;
    /// <summary>
    ...
    function GetChat(const ChatId: TtdUserLink): ItdChatFullInfo;
    /// <summary>
    ...
    /// <summary>
    /// <title>GetUserChatBoosts</title>
    /// Use this method to get the list of boosts added to a chat by a user. <br/>
    /// Requires administrator rights in the chat.<br/>
    /// </summary>
    /// <param name="ChatId">
    /// Unique identifier for the chat or username of the channel (in the format @channelusername <br/>
    /// </param>
    /// <param name="UserId">
    /// Unique identifier of the target user <br/>
    /// </param>
    /// <returns>
    /// Returns a UserChatBoosts object. <br/>
    /// </returns>
    /// <seealso href="https://core.telegram.org/bots/api#getuserchatboosts" />
...
{$REGION 'ForumTopic'}
    function getForumTopicIconStickers(): TArray<ItdSticker>;

    function createForumTopic(
      const chat_id: TtdUserLink;
      const name: string = '';
      const icon_color: integer = 0;
      const icon_custom_emoji_id: string = ''): ItdForumTopic;

    function editForumTopic(
      const chat_id: TtdUserLink;
      const message_thread_id: int64 = 0;
      const name: string = '';
      const icon_custom_emoji_id: string = ''): Boolean;

    function closeForumTopic(
      const chat_id: TtdUserLink;
      const message_thread_id: int64 = 0): Boolean;
...
{$REGION 'Business'}
  ...
  function getBusinessConnection(
    const business_connection_id: string = ''
...
{$REGION 'BotCommands'}
    ...
    function SetMyCommands(
        ...
        //const scope: ItdBotCommandScope = Nil;
        const scope: TtdBotCommandScopeType = TtdBotCommandScopeType.BotCommandScopeDefault;
        const language_code: string = ''): Boolean;

    function SetMyCommands2(
        const Command: TArray<TtdBotCommand>;
        const scope: ItdBotCommandScope = Nil;
        const language_code: string = ''): Boolean;
    /// <summary>
...
{$REGION 'BotDescription'}
    function setMyDescription(
        const description: string = ''; //0-512 characters
        const language_code: string = '' //two-letter ISO 639-1 language code
    ): Boolean;
    function getMyDescription(
        const language_code: string = '' //two-letter ISO 639-1 language code
    ): ItdBotDescription;
    function setMyShortDescription(
        const short_description: string = ''; //0-120 characters
        const language_code: string = '' //two-letter ISO 639-1 language code
    ): Boolean;
    function getMyShortDescription(
        const language_code: string = '' //two-letter ISO 639-1 language code
    ): ItdBotShortDescription;
{$ENDREGION 'BotDescription'}
...
{$REGION 'Updating messages'}
    ...
    function EditMessageText( //
      ...
      const Text: string; //
      const ParseMode: TtdParseMode = TtdParseMode.Default; //
      const Entities: TArray<TtdMessageEntity> = [];
      const LinkPreviewOptions : ItdLinkPreviewOptions = nil; //link_preview_options
    ...
    function EditMessageReplyMarkup( //
      ...
      ReplyMarkup: IReplyMarkup = nil): ItdMessage; overload;
    /// <summary>
    /// Use this method to delete a message, including service messages.  <br/>
    /// </summary>
    /// <param name="ChatId" type="Integer or String">
    /// Unique identifier for the target chat or username <br/>
    /// of the target channel (in the format @channelusername) <br/>
    /// </param>
    /// <param name="MessageId" type="Integer">
    ...
    /// - If the bot has can_delete_messages permission in a supergroup or a channel,  <br/>
    ///      it can delete any message there.  <br/>
    /// </remarks>
    ...
    /// <summary>
    /// Use this method to delete multiple messages simultaneously. <br/>
    /// </summary>
    /// <param name="ChatId" type="Integer or String">
    /// Unique identifier for the target chat or username <br/>
    /// of the target channel (in the format @channelusername) <br/>
    /// </param>
    /// <param name="MessageIds" type="TArray<Int64>">
    /// Identifier of the message to delete <br/>
    /// </param>
    /// <returns>
    /// Returns True on success. <br/>
    /// </returns>
    /// <remarks>
    /// If some of the specified messages can't be found, they are skipped. <br/>
    /// </remarks>
    /// <seealso href="https://core.telegram.org/bots/api#deletemessages" />
    function DeleteMessages( //
      const ChatId: TtdUserLink; //
      const MessageIds: TArray<Int64>): Boolean;
    /// <summary>
    ///  CopyMessage
    ...
    /// <summary>
    ///  CopyMessage
    /// Use this method to copy messages of any kind.<br/>
    /// </summary>
    /// <param name="ChatId">
    /// Unique identifier for the target chat or username of the target
    /// supergroup (in the format <c>@supergroupusername</c>) <br/>
    ...
    /// </param>
    /// <param name="MessageIds">
    /// Identifiers of 1-100 messages in the chat from_chat_id to copy. <br/>
    ///  The identifiers must be specified in a strictly increasing order. <br/>
    /// </param>
    /// <param name="DisableNotification">
    /// Envia a mensagem silenciosamente . Os usuários receberão uma notificação
    ...
    /// Pass True to copy the messages without their captions. <br/>
    /// </param>
    ///  <remarks>
    ///  If some of the specified messages can't be found or copied, they are <br/>
    ///  skipped. Service messages, giveaway messages, giveaway winners <br/>
    ///  messages, and invoice messages can't be copied. A quiz poll can be <br/>
    ///  copied only if the value of the field correct_option_id is known to <br/>
    ///  the bot. The method is analogous to the method forwardMessages, but <br/>
    ///  the copied messages don't have a link to the original message. <br/>
    ///  Album grouping is kept for copied messages. <br/>
    ///  </remarks>
    /// <returns>
    /// On success, an array of MessageId of the sent messages is returned.
    /// <a href="https://core.telegram.org/bots/api#copymessages">copyMessages</a>
    /// </returns>
    function CopyMessages( //
      ...
      const DisableNotification: Boolean = False; //
      const ProtectContent: Boolean = False;
      const RemoveCaption: Boolean = False): Int64;
{$ENDREGION}
...
{$REGION 'Inline mode'}
    ...
    function AnswerInlineQuery( //
      ...
      const Button: TtdInlineQueryResultsButton = nil): Boolean;
{$ENDREGION}
...
{$REGION 'Manage users and admins'}
    ...
    function PromoteChatMember( //
      ...
      const CanPostStories: Boolean = False; //
      const CanEditStories: Boolean = False; //
      const CanDeleteStories: Boolean = False;
      const CanManageTopics: Boolean = False ): Boolean;
    /// <summary>
...
{$REGION 'Strickers'}
    ...
    function uploadStickerFile(
      const UserId: Int64;
      const sticker: TtdFileToSend;
      const sticker_format: TtdStickerFormat): ItdFile;
    /// <summary>
    ...
    function createNewStickerSet( //
      ...
      const stickers: TArray<TtdInputSticker>;
      const Sticker_type: TtdStickerType; //
      const needs_repainting: Boolean = false): Boolean;
    /// <summary>
    ...
    function addStickerToSet( //
      const UserId: Int64; //
      const Name: string; //
      const sticker: TtdFileToSend): Boolean;
    /// <summary>
    ...
    /// <summary>
    /// Use this method to replace an existing sticker in a sticker set with a new one. The method is equivalent to calling deleteStickerFromSet, then addStickerToSet, then setStickerPositionInSet. <br/>
    /// </summary>
    /// <returns>
    /// Returns True on success.<br/>
    /// </returns>
    function replaceStickerInSet(
      ...
      const name:	String;
      const old_sticker:	String;
      const sticker:	ItdInputSticker
    ): Boolean;
    ...
    function setStickerSetThumbnail(
        ...
        const Format: TtdStickerFormat = TtdStickerFormat.static): Boolean;
    /// <summary>
    /// Use this method to get information about custom emoji stickers by their
    /// identifiers. <br/>
    /// </summary>
    /// <param name="custom_emoji_ids">
    /// List of custom emoji identifiers. At most 200 custom emoji identifiers can be specified. <br/>
    /// </param>
    /// <returns>
    /// Returns an Array of Sticker objects.
    /// </returns>
    function getCustomEmojiStickers(
        const custom_emoji_ids: TArray<String>): TArray<ItdSticker>;

    function setStickerMaskPosition(
      const sticker: string;
      const mask_position: ItdMaskPosition): Boolean;

    function setStickerKeywords(
      const sticker: string;
      const keywords: TArray<string>): Boolean;

    function setStickerEmojiList(
      const sticker: string;
      const emoji_list: TArray<string>): Boolean;

    function setStickerSetTitle(
      const name: string;  //requerido
      const title: string) : Boolean; //requerido

    function deleteStickerSet(
      const name: string) : Boolean; //requerido

    function setCustomEmojiStickerSetThumbnail(
      const name: string;
      const custom_emoji_id: string): Boolean;
{$ENDREGION}
...
{$ENDREGION 'TInjectTelegramBotBase'}
...
begin
  ...
  Result := GetRequest.SetMethod('setWebhook') //
    ...
    .AddParameter('secret_token', SecretToken, '', False) //
    .ExecuteAsBool;
...
function TInjectTelegramBot.CreateInvoiceLink(const Title, Description, Payload,
  ProviderToken, Currency: string; const Prices: TArray<TtdLabeledPrice>;
  const MaxTipAmount: Integer; const SuggestedTipAmounts: TArray<Integer>;
  const ProviderData, PhotoUrl: string; const PhotoSize, PhotoWidth,
  PhotoHeight: Int64; const NeedName, NeedPhoneNumber, NeedEmail,
  NeedShippingAddress, SendPhoneNumberToProvider, SendRmailToProvider,
  IsFlexible: Boolean): string;
Var
  LabeledPriceJson,
...
begin
  ...
  Logger.Enter(Self, 'CreateInvoiceLink');
  Result := GetRequest.SetMethod('createInvoiceLink') //
    ...
    .ExecuteAndReadValue;
  Logger.Leave(Self, 'CreateInvoiceLink');
end;
...
function TInjectTelegramBot.getCustomEmojiStickers(
  const custom_emoji_ids: TArray<String>): TArray<ItdSticker>;
begin
  Logger.Enter(Self, 'GetCustomEmojiStickers');
  Result := GetArrayFromMethod<ItdSticker>(TtdSticker,
    GetRequest.SetMethod('getCustomEmojiStickers') //
    .AddParameter('custom_emoji_ids',
        TJsonUtils.ArrayStringToJString(custom_emoji_ids), '[]', True) //
    .Execute);
  Logger.Leave(Self, 'GetCustomEmojiStickers');
end;
...
function TInjectTelegramBot.getMyDescription(
  const language_code: string): ItdBotDescription;
begin
  Logger.Enter(Self, 'GetMyDescription');
  Result := TtdBotDescription.Create(GetRequest.SetMethod('getMyDescription') //
    .AddParameter('language_code', language_code, '', False) //
    .Execute);
  Logger.Leave(Self, 'GetMyDescription');
end;


function TInjectTelegramBot.SetMyName(const name,
  language_code: string): Boolean;
begin
  Logger.Enter(Self, 'SetMyName');
  Result := GetRequest.SetMethod('setMyName') //
    .AddParameter('name', name, '', False) //
    .AddParameter('language_code', language_code, '', False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetMyName');
end;

function TInjectTelegramBot.GetMyName(const language_code: string): ItdBotName;
begin
  Logger.Enter(Self, 'GetMyName');
  Result := TtdBotName.Create(GetRequest.SetMethod('getMyName') //
    .AddParameter('language_code', language_code, '', False) //
    .Execute);
  Logger.Leave(Self, 'GetMyName');
end;

function TInjectTelegramBot.setMyDescription(const description,
  language_code: string): Boolean;
begin
  Logger.Enter(Self, 'SetMyDescription');
  Result := GetRequest.SetMethod('setMyDescription') //
    .AddParameter('description', description, '', False) //
    .AddParameter('language_code', language_code, '', False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetMyDescription');
end;

function TInjectTelegramBot.setMyShortDescription(const short_description,
  language_code: string): Boolean;
begin
  Logger.Enter(Self, 'SetMyShortDescription');
  Result := GetRequest.SetMethod('setMyShortDescription') //
    .AddParameter('short_description', short_description, '', False) //
    .AddParameter('language_code', language_code, '', False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetMyShortDescription');
end;

function TInjectTelegramBot.getMyShortDescription(
  const language_code: string): ItdBotShortDescription;
begin
  Logger.Enter(Self, 'GetMyShortDescription');
  Result := TtdBotShortDescription.Create(GetRequest.SetMethod('getMyShortDescription') //
    .AddParameter('language_code', language_code, '', False) //
    .Execute);
...
function TInjectTelegramBot.SendLocation(
  ...
  const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup;
  const ProtectContent: Boolean;
  const MessageThreadId: int64;
  const BusinessConnectionId: string): ItdMessage;
begin
...
begin
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('sendPhoto') //
    ...
    .AddParameter('has_spoiler', HasSpoiler, False, False) //
    .AddParameter('reply_markup', TInterfacedObject(ReplyMarkup), nil, False) //
    .Execute);
...
function TInjectTelegramBot.SendPoll( ////Add By Ruan Diego Lacerda Menezes 13/04/2020
  ...
  const QuestionEntities: TArray<TtdMessageEntity>): ItdMessage;
var
  LTmpJson, LTmpJson2, LTmpJson3: string;
begin
  Logger.Enter(Self, 'SendPoll');
  LTmpJson := TJsonUtils.ArrayToJString<TtdInputPollOption>(Options);
  LTmpJson2 := TJsonUtils.ArrayToJString<TtdMessageEntity>(ExplanationEntities);
  LTmpJson3 := TJsonUtils.ArrayToJString<TtdMessageEntity>(QuestionEntities);
  Result := TtdMessage.Create(GetRequest.SetMethod('sendPoll') //
    .AddParameter('business_connection_id ', BusinessConnectionId, '', False) //
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('question', Question, '', True) //
    .AddParameter('question_parse_mode', QuestionParseMode, '', False) //
    .AddParameter('options', LTmpJson, '[]', True) //
    .AddParameter('question_entities', LTmpJson3, '[]', False) //
    ...
    .AddParameter('explanation_entities', LTmpJson2, '[]', False) //
...
function TInjectTelegramBot.SendMessage( //
  const ChatId: TtdUserLink; //
  const Text: string; //
  const ParseMode: TtdParseMode = TtdParseMode.Default; //
  const LinkPreviewOptions : ItdLinkPreviewOptions = nil; //
...
begin
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('sendMessage') //
    ...
    .AddParameter('parse_mode', ParseMode.ToString, '', False) //
    .AddParameter('link_preview_options', TInterfacedObject(LinkPreviewOptions), nil, False) //
...
function TInjectTelegramBot.SendVenue(const ChatId: TtdUserLink; const Venue: ItdVenue;
  ...
  const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup; const ProtectContent: Boolean;
  const MessageThreadId: int64;
  const BusinessConnectionId: string): ItdMessage;
begin
...
function TInjectTelegramBot.SendVideo(const ChatId: TtdUserLink; const Video:
  ...
  DisableNotification: Boolean; const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup;
  const ProtectContent: Boolean;
  const MessageThreadId: int64;
  const CaptionEntities: TArray<TtdMessageEntity>;
...
function TInjectTelegramBot.SendVideoNote(const ChatId: TtdUserLink; const VideoNote:
  ...
  Boolean; const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup;
  const ProtectContent: Boolean;const MessageThreadId: int64;
  const BusinessConnectionId: string): ItdMessage;
begin
...
function TInjectTelegramBot.SendVoice(const ChatId: TtdUserLink; const Voice:
  TtdFileToSend; const Caption: string; const ParseMode: TtdParseMode; const
  Duration: Int64; const DisableNotification: Boolean;
  const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup; const ProtectContent: Boolean;
  const MessageThreadId: int64;
  const CaptionEntities: TArray<TtdMessageEntity>;
...
begin
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('sendAnimation') //
    ...
    .AddParameter('width', Width, 0, False) //
    .AddParameter('height', Height, 0, False) //
    .AddParameter('thumbnail', Thumbnail, nil, True) //
    .AddParameter('caption', Caption, '', False) //
    .AddParameter('parse_mode', ParseMode.ToString, '', False) //
    .AddParameter('disable_notification', DisableNotification, False, False) //
    .AddParameter('reply_parameters', TInterfacedObject(ReplyParameters), nil, False) //
...
function TInjectTelegramBot.SendChatAction(const ChatId: TtdUserLink;
  const Action: TtdSendChatAction; const MessageThreadId: int64;
  const BusinessConnectionId: string): Boolean;
begin
...
function TInjectTelegramBot.SendContact(const ChatId: TtdUserLink; const Contact:
  ItdContact; const DisableNotification: Boolean;
  const ReplyParameters: ItdReplyParameters;
  const ReplyMarkup: IReplyMarkup; const ProtectContent: Boolean;
  const MessageThreadId: int64;
  const BusinessConnectionId: string): ItdMessage;
begin
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('sendContact') //
    .AddParameter('business_connection_id ', BusinessConnectionId, '', False) //
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('phone_number', TtdContact(Contact).PhoneNumber, '', True) //
    .AddParameter('first_name', TtdContact(Contact).FirstName, '', True) //
    .AddParameter('last_name', TtdContact(Contact).LastName, '', False) //
    .AddParameter('vcard', TtdContact(Contact).VCard, '', False) //
    .AddParameter('disable_notification', DisableNotification, False, False) //
    .AddParameter('reply_parameters', TInterfacedObject(ReplyParameters), nil, False) //
    .AddParameter('protect_content ', ProtectContent, False, False) //
    .AddParameter('message_thread_id ', MessageThreadId, 0, False) //
...
function TInjectTelegramBot.GetUserChatBoosts(const ChatId: TtdUserLink;
  const UserId: Integer): ItdUserChatBoosts;
begin
  Logger.Enter(Self, 'GetUserChatBoosts');
  Result := TtdUserChatBoosts.Create(GetRequest.SetMethod('getUserChatBoosts') //
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('user_id', UserId, 0, True) //
    .Execute);
  Logger.Leave(Self, 'GetUserChatBoosts');
end;
...
function TInjectTelegramBot.SetMyCommands(
    const Command: TArray<TtdBotCommand>;
    const scope: TtdBotCommandScopeType;
    const language_code: string): Boolean;
Var
  LTmpJson: String;
begin
  ...
  Result := GetRequest.SetMethod('setMyCommands') //
    ...
    .AddParameter('scope', scope.ToJsonObject, TtdBotCommandScopeType.BotCommandScopeDefault.ToJsonObject, False)
    .AddParameter('language_code', language_code, '', False)
    .ExecuteAsBool;
...
function TInjectTelegramBot.SetMyCommands2(const Command: TArray<TtdBotCommand>;
  const scope: ItdBotCommandScope; const language_code: string): Boolean;
Var
  LTmpJson: String;
begin
  LTmpJson := TJsonUtils.ArrayCommandsToJString<TtdBotCommand>(Command);
  ...
  Result := GetRequest.SetMethod('setMyCommands') //
    .AddParameter('commands', LTmpJson, '[]', True)
    .AddParameter('scope', TtdBotCommandScope(scope).AsJson, TtdBotCommandScopeType.BotCommandScopeDefault.ToJsonObject, False)
    .AddParameter('language_code', language_code, '', False)
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetMyCommands');
...
function TInjectTelegramBot.GetMyCommands2(
  const scope: ItdBotCommandScope;
  const language_code: string): TArray<ItdBotCommand>;
begin
  Logger.Enter(Self, 'GetMyCommands');
  Result := GetArrayFromMethod<ItdBotCommand>(TtdBotCommand,
...
function TInjectTelegramBot.DeleteMyCommands2(const scope: ItdBotCommandScope;
  const language_code: string): Boolean;
begin
  Logger.Enter(Self, 'DeleteMyCommands');
  Result := GetRequest.SetMethod('deleteMyCommands') //
...
function TInjectTelegramBot.CopyMessages(const ChatId: TtdUserLink;
  const MessageThreadId: Integer; const FromChatId: TtdUserLink;
  const MessageIds: TArray<Int64>; const DisableNotification, ProtectContent,
  RemoveCaption: Boolean): Int64;
Var
  LTmpJson: String;
begin
  Logger.Enter(Self, 'CopyMessages');
  LTmpJson := TJsonUtils.ArrayInt64ToJString(MessageIds);
  Result := GetRequest.SetMethod('copyMessages') //
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('message_thread_id', MessageThreadId, 0, False) //
    .AddParameter('from_chat_id', FromChatId, 0, False) //
    .AddParameter('message_ids', LTmpJson, '[]', True) //
    .AddParameter('disable_notification', DisableNotification, False, False) //
    .AddParameter('protect_content ', ProtectContent, False, False) //
    .AddParameter('remove_caption ', RemoveCaption, False, False) //
    .ExecuteAsInt64('message_id');
  Logger.Leave(Self, 'CopyMessages');
...
function TInjectTelegramBot.ForwardMessages(const ChatId,
  FromChatId: TtdUserLink; const MessageIds: TArray<Int64>;
  const MessageThreadId: Integer; const DisableNotification,
  ProtectContent: Boolean): ItdMessage;
Var
  LTmpJson: String;
begin
  LTmpJson := TJsonUtils.ArrayInt64ToJString(MessageIds);

  Logger.Enter(Self, 'ForwardMessages');
  Result := TtdMessage.Create(GetRequest.SetMethod('forwardMessages')//
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('from_chat_id', FromChatId, 0, false) //
    .AddParameter('message_ids', LTmpJson, '[]', True) //
    .AddParameter('message_thread_id', MessageThreadId, 0, False) //
    ...
    .AddParameter('protect_content ', ProtectContent, False, False) //
    .Execute);
  Logger.Leave(Self, 'ForwardMessages');
end;

function TInjectTelegramBot.getBusinessConnection(
  const business_connection_id: string): TtdBusinessConnection;
begin
  Logger.Enter(Self, 'GetBusinessConnection');
  Result := TtdBusinessConnection.Create(GetRequest.SetMethod('getBusinessConnection').//
    AddParameter('business_connection_id', business_connection_id, '', True).Execute);
  Logger.Leave(Self, 'GetBusinessConnection');
end;

function TInjectTelegramBot.GetChat(const ChatId: TtdUserLink): ItdChatFullInfo;
begin
  Logger.Enter(Self, 'GetChat');
  Result := TtdChatFullInfo.Create(GetRequest.SetMethod('getChat').//
...
{$REGION 'ForumTopic'}
function TInjectTelegramBot.getForumTopicIconStickers: TArray<ItdSticker>;
var
  LRequest: ItdRequestAPI;
begin
  Logger.Enter(Self, 'GetForumTopicIconStickers');
  LRequest := GetRequest.SetMethod('getForumTopicIconStickers');
  Result := GetArrayFromMethod<ItdSticker>(TtdSticker, LRequest.Execute);
  Logger.Leave(Self, 'GetForumTopicIconStickers');
end;

function TInjectTelegramBot.unhideGeneralForumTopic(
  const chat_id: TtdUserLink): Boolean;
begin
  Logger.Enter(Self, 'UnhideGeneralForumTopic');
  Result := GetRequest.SetMethod('unhideGeneralForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'UnhideGeneralForumTopic');
end;

function TInjectTelegramBot.createForumTopic(const chat_id: TtdUserLink;
  const name: string; const icon_color: integer;
  const icon_custom_emoji_id: string): ItdForumTopic;
begin
  Logger.Enter(Self, 'createForumTopic');
  Result := TtdForumTopic.Create(GetRequest.SetMethod('createForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .AddParameter('name', name, '', True) //
    .AddParameter('icon_color', icon_color, 0, False) //
    .AddParameter('icon_custom_emoji_id', icon_custom_emoji_id, '', False) //
    .Execute);
  Logger.Leave(Self, 'createForumTopic');
end;

function TInjectTelegramBot.closeForumTopic(const chat_id: TtdUserLink;
  const message_thread_id: int64): Boolean;
begin
  Logger.Enter(Self, 'CloseForumTopic');
  Result := GetRequest.SetMethod('closeForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .AddParameter('message_thread_id', message_thread_id, 0, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'CloseForumTopic');
end;

function TInjectTelegramBot.closeGeneralForumTopic(
  const chat_id: TtdUserLink): Boolean;
begin
  Logger.Enter(Self, 'CloseGeneralForumTopic');
  Result := GetRequest.SetMethod('closeGeneralForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'CloseGeneralForumTopic');
end;

function TInjectTelegramBot.deleteForumTopic(const chat_id: TtdUserLink;
  const message_thread_id: int64): Boolean;
begin
  Logger.Enter(Self, 'DeleteForumTopic');
  Result := GetRequest.SetMethod('deleteForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .AddParameter('message_thread_id', message_thread_id, 0, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'DeleteForumTopic');
end;
function TInjectTelegramBot.editForumTopic(const chat_id: TtdUserLink;
  const message_thread_id: int64; const name: string;
  const icon_custom_emoji_id: string): Boolean;
begin
  Logger.Enter(Self, 'EditForumTopic');
  Result := GetRequest.SetMethod('editForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .AddParameter('message_thread_id', message_thread_id, 0, True) //
    .AddParameter('name', name, '', False) //
    .AddParameter('icon_custom_emoji_id', icon_custom_emoji_id, '', False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'EditForumTopic');
end;

function TInjectTelegramBot.editGeneralForumTopic(const chat_id: TtdUserLink;
  const name: string): Boolean;
begin
  Logger.Enter(Self, 'EditGeneralForumTopic');
  Result := GetRequest.SetMethod('editGeneralForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .AddParameter('name', name, '', True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'EditGeneralForumTopic');
end;
function TInjectTelegramBot.unpinAllForumTopicMessages(
  const chat_id: TtdUserLink; const message_thread_id: int64): Boolean;
begin
  Logger.Enter(Self, 'CloseForumTopic');
  Result := GetRequest.SetMethod('closeForumTopic') //
    .AddParameter('chat_id', chat_id, 0, True) //
...
function TInjectTelegramBot.unpinAllGeneralForumTopicMessages(
  const chat_id: TtdUserLink): Boolean;
begin
  Logger.Enter(Self, 'UnpinAllGeneralForumTopicMessages');
  Result := GetRequest.SetMethod('unpinAllGeneralForumTopicMessages') //
    .AddParameter('chat_id', chat_id, 0, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'UnpinAllGeneralForumTopicMessages');
end;

function TInjectTelegramBot.reopenForumTopic(const chat_id: TtdUserLink;
  const message_thread_id: int64): Boolean;
begin
  Logger.Enter(Self, 'CloseForumTopic');
  Result := GetRequest.SetMethod('closeForumTopic') //
...
function TInjectTelegramBot.hideGeneralForumTopic(
  const chat_id: TtdUserLink): Boolean;
begin
  Logger.Enter(Self, 'CloseForumTopic');
  Result := GetRequest.SetMethod('closeForumTopic') //
...
function TInjectTelegramBot.reopenGeneralForumTopic(
  const chat_id: TtdUserLink): Boolean;
begin
  Logger.Enter(Self, 'CloseForumTopic');
  Result := GetRequest.SetMethod('closeForumTopic') //
...
function TInjectTelegramBot.EditMessageText(const InlineMessageId, Text: string; const
  ...
  const Entities: TArray<TtdMessageEntity>;
  const LinkPreviewOptions : ItdLinkPreviewOptions; //link_preview_options
  ReplyMarkup: IReplyMarkup): ItdMessage;
Var
  LTmpJson: String;
begin
  LTmpJson := TJsonUtils.ArrayToJString<TtdMessageEntity>(Entities);
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('editMessageText') //
    ...
    .AddParameter('text', Text, '', True) //
    .AddParameter('parse_mode', ParseMode.ToString, '', False) //
    .AddParameter('entities', LTmpJson, '', False) //
    .AddParameter('link_preview_options', TInterfacedObject(LinkPreviewOptions), nil, False) //
    .AddParameter('reply_markup', TInterfacedObject(ReplyMarkup), nil, False) //
    .Execute);
  Logger.Leave(Self, 'EditMessageText');
end;
function TInjectTelegramBot.EditMessageText(const ChatId: TtdUserLink;
  const MessageId: Int64; const Text: string;
  const ParseMode: TtdParseMode;
  const Entities: TArray<TtdMessageEntity>;
  const LinkPreviewOptions : ItdLinkPreviewOptions;
  ReplyMarkup: IReplyMarkup): ItdMessage;
Var
  LTmpJson: String;
begin
...
function TInjectTelegramBot.DeleteMessages(const ChatId: TtdUserLink;
  const MessageIds: TArray<Int64>): Boolean;
Var
  LTmpJson: String;
begin

  if Length(MessageIds) > 100 then
  Begin
    Logger.Enter(Self, 'DeleteMessages');
    Exception.Create('The limit of 100 messages was exceeded in '+Length(MessageIds).ToString+', remove '+#10#13+
      'some messages from your array listing so that the method can be executed');
    Exit;
    Logger.Leave(Self, 'DeleteMessages');
  End Else
  Begin
    ...
    Logger.Enter(Self, 'DeleteMessages');
    Result := GetRequest.SetMethod('deleteMessages') //
      .AddParameter('chat_id', ChatId, 0, True) //
      .AddParameter('MessageIds', LTmpJson, '[]', True) //
      .ExecuteAsBool;
    Logger.Leave(Self, 'DeleteMessages');
  End;
...
function TInjectTelegramBot.EditMessageCaption(const ChatId: TtdUserLink;
  const MessageId: Int64;
  const Caption: string;
  const ParseMode: TtdParseMode;
  const CaptionEntities: TArray<TtdMessageEntity>;
...
function TInjectTelegramBot.setCustomEmojiStickerSetThumbnail(const name,
  custom_emoji_id: string): Boolean;
begin
  Logger.Enter(Self, 'SetCustomEmojiStickerSetThumbnail');
  Result := GetRequest.SetMethod('setCustomEmojiStickerSetThumbnail') //
    .AddParameter('name', name, '', True) //
    .AddParameter('custom_emoji_id', custom_emoji_id, '', True) //
    .ExecuteAsBool;
...
begin
  ...
  Result := GetRequest.SetMethod('promoteChatMember') //
    ...
    .AddParameter('can_post_stories', CanPostStories, False, False) //
    .AddParameter('can_delete_stories', CanEditStories, False, False) //
    .AddParameter('can_delete_stories', CanDeleteStories, False, False) //
    .AddParameter('can_manage_topics', CanManageTopics, False, False) //
    .ExecuteAsBool;
...
begin
  ...
  Result := GetRequest.SetMethod('addStickerToSet') //
    .AddParameter('user_id', UserId, 0, True) //
    .AddParameter('name', Name, '', False) //
    .AddParameter('sticker', sticker, nil, False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'addStickerToSet');
end;
function TInjectTelegramBot.createNewStickerSet(
  const UserId: Int64;
  const Name, Title: string;
  const stickers: TArray<TtdInputSticker>;
  const Sticker_type: TtdStickerType; //
...
Var
  LTmpJson: String;
begin
  LTmpJson := TJsonUtils.ArrayToJString<TtdInputSticker>(stickers);
  ...
  Result := GetRequest.SetMethod('createNewStickerSet') //
    ...
    .AddParameter('stickers', LTmpJson, '[]', False) //
    .AddParameter('Sticker_type', Sticker_type.ToString, '', False) //
    .AddParameter('needs_repainting', needs_repainting, False, True) //
    .ExecuteAsBool;
...
function TInjectTelegramBot.deleteStickerSet(const name: string): Boolean;
begin
  Logger.Enter(Self, 'DeleteStickerSet');
  Result := GetRequest.SetMethod('deleteStickerSet') //
    .AddParameter('name', name, '', True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'DeleteStickerSet');
end;
...
function TInjectTelegramBot.SendSticker(const ChatId: TtdUserLink; const Sticker:
  ...
  Int64; ReplyMarkup: IReplyMarkup; const ProtectContent: Boolean;
  const MessageThreadId: int64;
...
begin
  ...
  Result := TtdMessage.Create(GetRequest.SetMethod('sendSticker') //
    ...
    .AddParameter('emoji ', emoji, '', False) //
    .AddParameter('reply_markup', TInterfacedObject(ReplyMarkup), nil, False) //
    .Execute);
  Logger.Leave(Self, 'SendSticker');
end;
function TInjectTelegramBot.setStickerEmojiList(const sticker: string;
  const emoji_list: TArray<string>): Boolean;
begin
  Logger.Enter(Self, 'SetStickerEmojiList');
  Result := GetRequest.SetMethod('setStickerEmojiList') //
    .AddParameter('sticker', sticker, '', True) //
    .AddParameter('emoji_list', TJsonUtils.ArrayStringToJString(emoji_list), '[]', True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetStickerEmojiList');
end;

function TInjectTelegramBot.setStickerKeywords(const sticker: string;
  const keywords: TArray<string>): Boolean;
begin
  Logger.Enter(Self, 'SetStickerKeywords');
  Result := GetRequest.SetMethod('setStickerKeywords') //
    .AddParameter('sticker', sticker, '', True) //
    .AddParameter('keywords', TJsonUtils.ArrayStringToJString(keywords), '[]', True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetStickerKeywords');
end;

function TInjectTelegramBot.setStickerMaskPosition(const sticker: string;
  const mask_position: ItdMaskPosition): Boolean;
begin
  Logger.Enter(Self, 'SetStickerMaskPosition');
  Result := GetRequest.SetMethod('setStickerMaskPosition') //
    .AddParameter('sticker', name, '', True) //
    .AddParameter('mask_position', TInterfacedObject(mask_position), nil, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetStickerMaskPosition');
end;
...
function TInjectTelegramBot.setStickerSetThumbnail(const Name: string;
  const UserId: Int64; const Thumbnail: string;
  const Format: TtdStickerFormat): Boolean;
begin
  Logger.Enter(Self, 'SetStickerSetThumbnail');
  Result := GetRequest.SetMethod('setStickerSetThumbnail') //
    .AddParameter('name	', Name, '', True)
    .AddParameter('user_id', UserId, 0, True)
    .AddParameter('format', Format.ToString, '', True)
    .AddParameter('thumbnail', Thumbnail, '', False).ExecuteAsBool;
  Logger.Leave(Self, 'SetStickerSetThumbnail');
end;
function TInjectTelegramBot.setStickerSetTitle(const name,
  title: string): Boolean;
begin
  Logger.Enter(Self, 'SetStickerSetTitle');
  Result := GetRequest.SetMethod('setStickerSetTitle') //
    .AddParameter('name', name, '', True) //
    .AddParameter('title', title, '', True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'SetStickerSetTitle');
end;

function TInjectTelegramBot.uploadStickerFile(
      const UserId: Int64;
      const sticker: TtdFileToSend;
      const sticker_format: TtdStickerFormat): ItdFile;
begin
  ...
  Result := TtdFile.Create(GetRequest.SetMethod('uploadStickerFile') //
    .AddParameter('user_id', UserId, 0, True) //
    .AddParameter('sticker', sticker, nil, True) //
    .AddParameter('sticker_format', sticker_format.ToString, '', True) //
    .Execute);
  Logger.Leave(Self, 'uploadStickerFile');
end;

function TInjectTelegramBot.replaceStickerInSet(const user_id: Integer;
  const name, old_sticker: String; const sticker: ItdInputSticker): Boolean;
begin
  Logger.Enter(Self, 'ReplaceStickerInSet');
  Result := GetRequest.SetMethod('replaceStickerInSet') //
    .AddParameter('user_id', user_id, 0, True) //
    .AddParameter('name', name, '', True) //
    .AddParameter('old_sticker', old_sticker, '', True) //
    .AddParameter('sticker', TInterfacedObject(sticker), nil, True) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'ReplaceStickerInSet');
end;
{$ENDREGION}

{$REGION 'Inline mode'}
function TInjectTelegramBot.AnswerInlineQuery(
  const InlineQueryId: string;
  const Results: TArray<TtdInlineQueryResult>;
  const CacheTime: Int64;
  const IsPersonal: Boolean;
  const NextOffset: string;
  const Button: TtdInlineQueryResultsButton): Boolean;
begin
  ...
  Result := GetRequest.SetMethod('answerInlineQuery') //
    ...
    .AddParameter('button', Button, nil, False) //
    .ExecuteAsBool;
...
function TInjectTelegramBot.setMessageReaction(const ChatId: TtdUserLink;
  const MessageId: int64; const Reaction: TArray<TtdReactionType>;
  const IsBig: boolean): Boolean;
begin
  Logger.Enter(Self, 'SetMessageReaction');
  Result := GetRequest.SetMethod('setMessageReaction') //
    .AddParameter('chat_id', ChatId, 0, True) //
    .AddParameter('message_id', MessageId, 0, True) //
    .AddParameter('reaction', TJsonUtils.ArrayToJString<TtdReactionType>(Reaction), '[]', False) //
    .AddParameter('is_big', IsBig, False, False) //
    .ExecuteAsBool;
  Logger.Leave(Self, 'setMessageReaction');
end;
...
const
  TOKEN_CORRECT = '[-+]?[0-9]{10}\:?[\w\d-_]{35}'; //'/\d*:[\w\d-_]{35}/';
begin
