...
type
  ...
  TtdBotCommandScopeHelper = record helper for TtdBotCommandScopeType
  ...
  TtdStickerFormatHelper = record helper for TtdStickerFormat
    function ToString: String;
  End;
...
begin
  ...
  try
    ...
    if TAllowedUpdate.ChatBoost in Self then
      LAllowed.Add('"chat_boost"');
    if TAllowedUpdate.RemovedChatBoost in Self then
      LAllowed.Add('"removed_chat_boost"');
...
{ TODO 5 -oDiego -cTipos de Dados : Promover a Integração do metodo de leitura de lista de classes para correção deste tipo de dado }
  case Self of
    TtdBotCommandScopeType.BotCommandScopeDefault:
      Result := '{"type":"default"}';
    TtdBotCommandScopeType.BotCommandScopeAllPrivateChats:
      Result := '{"type":"all_private_chats"}';
    TtdBotCommandScopeType.BotCommandScopeAllGroupChats:
      Result := '{"type":"all_group_chats"}';
    TtdBotCommandScopeType.BotCommandScopeAllChatAdministrators:
      Result := '{"type":"all_chat_administrators"}';
    TtdBotCommandScopeType.BotCommandScopeChat:
      Result := '{"type":"chat"}';
    TtdBotCommandScopeType.BotCommandScopeChatAdministrators:
      Result := '{"type":"chat_administrators"}';
    TtdBotCommandScopeType.BotCommandScopeChatMember:
...
{ TtdStickerFormatHelper }

function TtdStickerFormatHelper.ToString: String;
begin
  case Self of
    TtdStickerFormat.static: Result :=  'static';
    TtdStickerFormat.animated: Result :=  'animated';
    TtdStickerFormat.video: Result :=  'video';
  end;
end;
