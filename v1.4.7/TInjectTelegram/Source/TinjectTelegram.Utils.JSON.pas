unit TinjectTelegram.Utils.Json;
interface
{$I ..\Source\config.inc}
uses
...
type
  ...
  protected
    ...
    function ReadToClassArray<T: class, constructor>(const AKey: string): TArray<T>;
    function ReadToInterfaceArray<TI: IInterface>(TdClass: TBaseJsonClass; const AKey:
      string): TArray<TI>;
    function ReadToArraySimpleType<T>(const AKey: string): TArray<T>;
    function ReadToSimpleType<T>(const AKey: string): T;
    function ReadToDateTime(const AKey: string): TDateTime;
    function ReadToTArrayString(const AKey: string): TArray<string>;
  ...
  public
    ...
    class function GetTdClass: TBaseJsonClass; virtual; // abstract;
  ...
  TJsonUtils = class
    ...
    class function ArrayCommandsToJString<T: class>(LArray:  TArray<T>): string; static;
    ...
    class function ArrayIntToJString(LArray: TArray<Integer>): string;
    class function ArrayInt64ToJString(LArray: TArray<Int64>): string;
...
class function TJsonUtils.ArrayCommandsToJString<T>(LArray: TArray<T>): string;
Var
  I: Integer;
begin
  result := '[';
  for I := Low(LArray) to High(LArray) do
  begin
    if ((LArray[I] as TtdBotCommand).CMD <> '') and (Length((LArray[I] as TtdBotCommand).CMD) <= 32) then
      Result := Result + '{"command":"' + (LArray[I] as TtdBotCommand).CMD + '"'
    else
      raise Exception.Create('Error : O número de caracteres do comando não pode ser superior a 32!');
    if ((LArray[I] as TtdBotCommand).Desc <> '' ) and
      (Length((LArray[I] as TtdBotCommand).Desc) >= 3) and
      (Length((LArray[I] as TtdBotCommand).Desc) <= 256) then
      Result := Result + ',"description":"' +(LArray[I] as TtdBotCommand).Desc +'"}'
        else
      raise Exception.Create('Error : O número de caracteres da descrição não pode ser menor que 3 e maior que 256 caracteres!');
     if I <> High(LArray) then
       result := result +',';
  End;
  Result := Result + ']';
end;
class function TJsonUtils.ArrayInt64ToJString(LArray: TArray<Int64>): string;
var
  I: Integer;
begin
  Result := '[';
  for I := Low(LArray) to High(LArray) do
    if LArray[I].ToString <> '' then
    begin
      if (I = Low(LArray)) then
        Result := Result + LArray[I].ToString
      Else
        Result := Result + ',' + LArray[I].ToString;
    end;
  Result := Result + ']';
...
class function TJsonUtils.ArrayIntToJString(LArray: TArray<Integer>): string;
var
  I: Integer;
begin
  Result := '[';
...
begin
  ...
  for I := Low(LArray) to High(LArray) do
  begin
    if (LArray[I] as TtdLabeledPrice).sLabel <> '' then
      Result := Result + '"label":"' + (LArray[I] as TtdLabeledPrice).sLabel + '"';
    if (LArray[I] as TtdLabeledPrice).sAmount.ToString <> '' then
      Result := Result + ',"amount":"' +(LArray[I] as TtdLabeledPrice).sAmount.ToString +'"}' ;
     if I <> High(LArray) then
       result := result +',';
  End;
  Result := Result + '}]';
...
function TBaseJson.ReadToInterfaceArray<TI>(TdClass: TBaseJsonClass; const AKey: string):
  TArray<TI>;
var
...
// valor do cache para uso posterior
  ...
  if TdClass.GetInterfaceEntry(GUID) = nil then
    raise Exception.Create('TinjectTelegram.Utils.Json unit Error - check this '+ #10#13+ 'function TBaseJson.ReadToArray<TI>(TdClass: TBaseJsonClass; const AKey: string): TArray<TI>;'+ #10#13+ ' unsupported interface for ' +
      TdClass.ClassName);
  ...
  begin
    TdClass.GetTdClass.Create(LJsonArray.Items[I].ToString).GetInterface(GUID, Result[I]);
  end;
end;
function TBaseJson.ReadToArraySimpleType<T>(const AKey: string): TArray<T>;
var
  LJsonArray: TJSONArray;
  I: Integer;
begin
  ...
  try
    ...
    for I := 0 to LJsonArray.Count - 1 do
      Result[I] := ReadToSimpleType<T>(LJsonArray.Items[I].ToString);
  finally
    LJsonArray.Free;
  end;
...
begin
  ...
  begin
{$IFDEF USE_INDY}
    // A diretiva não se encaixa bem. Este é o caso se você estiver usando a versão antiga da IDE
...
function TBaseJson.ReadToClassArray<T>(const AKey: string): TArray<T>;
var
  LValue: string;
  LJsonArray: TJSONArray;
  I: Integer;
...
class function TBaseJson.GetTdClass: TBaseJsonClass;
begin
  Result := Self;
end;
...
function TBaseJson.ReadToTArrayString(const AKey: string): TArray<string>;
var
  LJsonArray: TJSONArray;
  I: Integer;
begin
...
begin
  raise Exception.Create('Método do Telegram não suportado na Biblioteca TinjectTelegram. Desculpe!');
end;
